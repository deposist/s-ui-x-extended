...
## [1.0.1] - 2026-07-07 - стабильный релиз 1.0.1

Стабильная v1.0.1 повышает beta-линейку v1.0.1 до stable и добавляет исправление настройки Telegram Chat ID. Ручная миграция базы данных или конфигурации не требуется.
...
- Включены hardening-изменения beta-линейки v1.0.1: проверка владения API-токеном, safer default `subSecretRequired` для новых установок, forced password reset sessions, cleanup `reserved` fields для WARP/WireGuard и миграция inbound sniff.

Полные release notes: [`docs/releases/v1.0.1.md`](docs/releases/v1.0.1.md).

## [1.0.1-beta5] - 2026-07-02 - усиление безопасности админки

Релиз с усилением безопасности. Ручная миграция базы не требуется.

- Удаление и включение/выключение API-токена теперь требуют владения токеном.
- Новые установки теперь используют `subSecretRequired=true` по умолчанию; существующие установки сохраняют своё значение.
- Админы с `force_password_reset=true` теперь получают ограниченную сессию и должны сменить пароль перед доступом к панели.
