...
## [1.0.2] - 2026-07-14 - стабильный релиз 1.0.2

- Добавлены управляемые персональные устройства AmneziaWG 2.0: зашифрованные ключи, лимиты по тарифам, выдача конфигурации и QR через Telegram, учёт трафика, ротация, отзыв и восстановление после сбоев.
- Пиры управляемого endpoint защищены от ручного редактирования. Preshared keys передаются в core только в памяти.
- Исправлено зависание `s-ui` перед показом меню, если язык уже сохранён.
- Загрузка релиза усилена HTTPS-only режимом curl, повторами, тайм-аутами и обязательной проверкой SHA-256.

Полные release notes: [`docs/releases/v1.0.2.md`](docs/releases/v1.0.2.md).
