  ...
  <div class="outbounds-nexus">
    ...
    <nexus-data-table :columns="columns" :items="filtered" :row-key="(item) => item.id">
      ...
      <template #empty>
        ...
        <empty-state
          v-else
          :description="$t('emptyState.outboundsHint')"
          icon="lucide:arrow-up-right"
          :title="$t('emptyState.outbounds')"
        >
          <template #action>
            <v-btn color="primary" prepend-icon="lucide:plus" variant="flat" @click="emit('add')">
              {{ $t('actions.add') }}
...
// they simply have no outbounds yet would send them down the wrong path.
const showNoMatches = computed(
  () => search.value.trim().length > 0 && props.outbounds.length > 0,
)
