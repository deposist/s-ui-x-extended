...
Расширяет встроенную миграцию 3x-ui (`migrate-xui`) на proxy Outbounds, гораздо
...
- **Proxy Outbounds теперь мигрируют как Outbounds в s-ui** - раньше
  ...
  `http` молча отбрасывалась, поэтому цепочечный/proxy Outbound пропадал из
  перенесённой панели. Теперь каждый такой Outbound превращается в полноценный
  Outbound в s-ui (sing-box): сервер/порт, учётные данные (`uuid`/`password`/
  ...
  Outbound также регистрируется как цель маршрутизации, поэтому правило,
  ссылавшееся на него, теперь резолвится в перенесённый Outbound вместо пометки
  «требует ручной проверки».
- **Системные Outbounds сопоставляются со своим аналогом в sing-box** -
  `freedom`→`direct` и `blackhole`→`block` (как и раньше), а Outbound `dns`
  теперь превращается в действие маршрута `hijack-dns` (в sing-box нет
  Outbound `dns`). `loopback` и любой протокол, которого нет в Xray (например,
...
- **Никакой тихой потери при выключенном импорте роутинга** - прокси/WARP
  Outbounds лежат в исходном `xrayConfig`, который читается только при импорте
  роутинга. Если импорт роутинга выключен, но в источнике есть Outbounds, план
...
- Outbounds создаются только если они новые - повторный импорт или
  ...
  Outbound с тем же тегом. В отчёте об импорте появляется счётчик `outbounds`
...
- **Не-reality TLS-сертификаты мигрируют** - Inbound, у которого в `tlsSettings`
...
- **Мелкие улучшения точности** - WebSocket-транспорт переносит все заголовки
  ...
  источник его использовал; Outbound с несколькими серверами превращается в
...
- **Trojan Inbound больше не роняет ядро** (не связано с миграцией) - редактор
  Inbound писал верхнеуровневый `password` для Trojan Inbounds, но у sing-box
  ...
  Trojan *Outbounds* (где оно валидно), а сборщик конфига срезает оставшийся
  верхнеуровневый `password` у Trojan Inbound - так что уже созданные Inbounds
...
proxy Outbounds, правила маршрутизации и блок DNS; проверьте всё, что отчёт
