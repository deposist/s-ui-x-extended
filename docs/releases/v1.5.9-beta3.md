...
* Only a signed-in administrator can update, and the action requires password re-entry. The UI warns that the panel and proxying will restart briefly, interrupting active client connections until the service is back.
...
* Обновление доступно только вошедшему администратору и требует повторного ввода пароля. Интерфейс предупреждает, что панель и проксирование кратковременно перезапустятся, а активные клиентские соединения восстановятся после запуска сервиса.
