...
// Ported from shtorm-7/sing-box-extended include/registry.go (tag
// v1.13.12-extended-2.4.0), adapted to s-ui-x:
//   - package `core` (not `include`);
//   - the `Context` helper lives in box.go (kept local), so it is omitted here;
//   - the centralized fleet-management services (admin_panel, manager,
//     manager_api, node, node_manager_api) are deliberately NOT registered —
//     they duplicate s-ui-x's own panel/auth/DB (see PLAN-s-ui-x-extended.md).
// The build-tag helper functions (registerQUICInbounds, registerMTProxyInbound,
// registerWireGuardEndpoint, registerProfilerService, ...) are defined in the
// sibling *.go/*_stub.go files copied from the fork's include/ directory, plus
// the pre-existing register_naive*.go / register_tailscale*.go.
