...
- The certificate providers are separate from the terminal menu's certificate tools: the menu installs the external acme.sh and issues certificates for the panel's own web interface, with the panel stopped during issuance and file paths wired by hand. Providers run inside the sing-box core, issue certificates for inbound protocols, renew them automatically and are referenced by tag. Tailscale and Cloudflare Origin CA providers have no terminal-menu counterpart.
- USB/IP, in one paragraph: it forwards USB devices over the network, so a device plugged into one machine appears on another as a local USB device. The usbip-server service runs where the device is plugged in (listen address, then a hand-written device list or automatic export), the usbip-client service runs where the device is needed (server address, devices picked by bus ID, vendor ID, product ID or serial). Linux needs the vhci driver and device access rights, which the core reports at startup.
...
Full release notes: [`docs/releases/v1.0.9-beta4.md`](docs/releases/v1.0.9-beta4.md).

## [1.0.9-beta3] - 2026-08-07 - database restore deadlock fix

- Database restore no longer deadlocks behind an open realtime WebSocket. The socket previously held a shared maintenance lease for its whole lifetime, so the exclusive restore lock could never be acquired. The WebSocket now releases the lease right after the handshake, before the connection loop.
- Cron maintenance jobs no longer block or spam `cron: skip` while a restore runs. A tick that collides with a restore is skipped silently, and the job runs on the next tick.
...
Full release notes: [`docs/releases/v1.0.9-beta3.md`](docs/releases/v1.0.9-beta3.md).
...
- Selective backups use one source snapshot and validate SQLite integrity, foreign keys, and the TLS sentinel before export.
- Traffic, IP-monitor, token-use, and audit batches keep pending data until the database commit succeeds. Failed writes are retried or requeued.
...
- Sudoku and mieru now produce a share link and QR. Sudoku is JSON-delivered and had no URI link, so the raw-links tab was empty; it now emits `sudoku://` (base64url of the short-link JSON, matching `SUDOKU-ASCII/sudoku`). Mieru emits `mierus://` (the `enfein/mieru` simple URL), chosen over `mieru://` because it needs no protobuf schema. Both stay `clientDelivery=json`, so the subscription path and the issue #4 assignability fix are unchanged; the link is an addition.
- Sudoku http-mask mode is now easy to find. The inbound fields sit in a titled card matching the outbound editor, the selector has a placeholder and a hint (empty means the core default, legacy, and the client only follows the mode if its core supports it), and the recommended preset fills in `http_mask_mode=legacy`. A regression test locks legacy/stream/poll/auto/ws into the client out_json.
- The raw-links tab no longer reads like a failure for JSON-delivered protocols. It now explains that some protocols have no URI link and are delivered through the JSON subscription, and points to the Sing-box tab for import or QR.
- Menu item 23 in `s-ui` is now "Generate env keys" with a submenu: session cookie key (`SUI_COOKIE_KEY`) and AmneziaWG device key (`AWG_KEY_ENC`). Web-panel self-update replaces the binary but does not run `install.sh`, so it never provisions `AWG_KEY_ENC`; the menu generates the key when it is missing from `/etc/s-ui/secretbox.env`, wires up the systemd drop-in, and offers a restart. An existing key is kept unless a rotation is confirmed, because rotating it after devices exist makes their stored key material undecryptable.
- Fixed `write_env_value` in the console script appending a duplicate line instead of replacing the value when an env key already existed; this also affected rotating `SUI_COOKIE_KEY` from the menu. Added `scripts/setup-awg-key.sh` for the same check-and-generate as a standalone command.
