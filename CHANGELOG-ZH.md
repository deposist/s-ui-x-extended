...
- 修复受管 AWG 端点的热重载：保存端点（例如切换 AmneziaWG 3.1 开关）时，面板在运行中的核心里重建端点，却没有带上客户端的预共享密钥 - 数据库里的对端记录不含 PSK，只有核心完整启动路径会注入密钥。此后所有客户端的握手都会失败，直到重启 s-ui 服务。热重载现在与完整启动一样注入 PSK；集成测试在重载后完成真实握手，并检查活核心中的 PSK 和两个 3.1 标志。
- 发布流水线在现有 AWG 2.0 测试之外，同时运行 AmneziaWG 3.1 随机尾部线上格式的集成测试。

完整发布说明：[`docs/releases/v1.1.1-beta9.md`](docs/releases/v1.1.1-beta9.md)。


## [1.1.1-beta8] - 2026-09-17 - 修复 AmneziaWG 3.1 随机尾部

- 修复开启「随机尾部」（3.1）后的握手故障：beta7 中开启该开关后没有任何客户端能连接，面板也看不到任何握手。引擎把握手报文写入「报文加尾部」长度的缓冲区，而编组代码要求精确的报文尺寸并返回长度错误，调用方忽略了这个错误，发出的包里没有握手内容。
- 引擎重新固定到 `deposist/wireguard-go v0.0.5-extended-1.6.2`（提交 `8f4b19e`，`shtorm-7/wireguard-go v1.6.1` 的分支）：发起、响应和 cookie-reply 现在编组到精确尺寸的切片，编组失败会中止发送。引擎的回环测试和面板针对真实核心的集成测试覆盖了带尾部的线上格式。
- 「禁用 cookies」不受影响。AmneziaVPN 5.0.1.5 及以上的客户端照常连接；该开关仍需在服务器和所有客户端保持一致。无需数据库迁移。

完整发行说明：[`docs/releases/v1.1.1-beta8.md`](docs/releases/v1.1.1-beta8.md)。
...
- 修复 TrustTunnel 和 Mieru 客户端订阅缺少密码的问题：凭据回填在创建客户端配置块时只写入客户端名称，而订阅构建器会丢弃空的凭据字段。官方 TrustTunnel 客户端依然显示已连接（TLS 握手发生在第一次授权请求之前），但服务器以 `authorization failed` 拒绝每个请求。现在创建配置块时会生成随机密码，配置块已存在时会补上空缺的密码；重新保存入站即可修复受影响的客户端。
- 修复入站保存导致 sing-box 因 `json: unknown field` 反复重启的问题：TrustTunnel 不再写入仅出站支持的 `quic` 开关，Trojan 不再写入 `network`，Sudoku 按入站结构保存 HTTP-mask 设置，Call 不再添加监听字段。生成核心配置时也会清理已有的受影响记录。
- 面板会在提交保存前校验最终生成的入站配置。恢复数据库时，会在完成恢复并重启 sing-box 前校验完整核心配置；无效备份会自动回滚到原数据库。
- 依赖 `github.com/pion/stun/v3` 从 3.1.1 升级到 3.1.5（GO-2026-6163，解析畸形的 XOR-MAPPED-ADDRESS 属性时 panic）。面板没有调用受影响的符号。
...
- 页面 `lang` 属性现在跟随所选界面语言，而不是硬编码的 `zh-CN`。屏幕阅读器可以选择正确的发音；应用加载保存的语言之前该属性为 `en`。
- 自更新模块的制品下载器拒绝把传输从 HTTPS 降到明文 HTTP 的重定向。
- 来自 GitHub API 的发布标签必须归一化为有效版本号后才能用于下载 URL；未通过校验的标签按无发布版本处理。此前原始标签仅经过 `TrimSpace` 就直接代入 URL 模板。
- 新增测试覆盖存储包装器、重定向拒绝和标签校验。

完整发布说明：[`docs/releases/v1.1.0-beta1.md`](docs/releases/v1.1.0-beta1.md)。
...
完整发布说明：[`docs/releases/v1.0.9-beta4.md`](docs/releases/v1.0.9-beta4.md)。

## [1.0.9-beta3] - 2026-08-07 - 修复数据库恢复死锁

- 面板打开时，实时 WebSocket 曾在其整个生命周期内持有数据库的共享维护锁，而恢复数据库前需要获取独占锁，因此恢复永远不会开始：面板卡死、API 请求排队、定时任务每两秒记录一次 `cron: skip`，只能手动重启服务恢复。
- 现在 WebSocket 在握手结束后立即释放锁；恢复进行期间，定时维护任务会静默跳过本周期，并在下一周期执行。
- 从面板发起的恢复可在面板保持打开的情况下完成，无需重启服务。
- 无需迁移数据库或手动修改配置。

完整发布说明：[`docs/releases/v1.0.9-beta3.md`](docs/releases/v1.0.9-beta3.md)。
...
- 自更新会在替换二进制文件前校验 HTTPS 发布元数据、版本、通道、平台、归档名称、SHA-256、大小限制和解压安全性。持久化事务标记会在更新中断或回滚时保护二进制文件和数据库。
- 紧凑的 `betaN` 和 `rcN` 后缀现在按数字排序。启动时还会迁移 beta8 写入的数字格式 `.update-pending` 文件；损坏的恢复状态仍会阻止启动。
- 付费订阅会持久记录每笔供应商扣款，在等待超时后恢复已确认订单，重试未完成的 Telegram 和 CryptoBot 工作，并在退款时正确恢复已购额度。
- 数据库恢复会等待面板、订阅和定时任务结束后再替换 SQLite。备份导出会检查完整性和外键；计数器、审计、IP 监控和令牌使用记录写入失败后会保留并重试。
- 区域路由和 DNS 预设会通过 HTTPS 下载并校验本地 `.srs` 文件，并每天刷新。下载器会拒绝私有地址重定向、DNS 重绑定、过大文件和无效规则集。路由与 DNS 编辑器支持有深度和节点数量限制的嵌套 `and`、`or` 组。
- 面板改善了窄屏布局、空状态、保存按钮禁用原因和无障碍支持，并修复 WebSocket 恢复、过期请求、CSRF 刷新、轮询和损坏浏览器存储的处理。
- Core 可以安全处理部分启动失败和并发关闭。Clash 日志、证书续期、WireGuard IPC 删除、订阅解析、链接生成、会话、实时事件、重定向和缓存响应头也修复了已知边界问题。
- 内置 core 为 `sing-box-extended v1.13.14-extended-2.5.4`。本版本还更新了受影响的 Go、QUIC、Axios、gRPC、文本处理和前端构建依赖。
- 发布任务会校验 tag 来源和构件元数据，构建 Linux 与 Windows 包，核对校验和及归档内容，扫描发布文件，生成 SPDX SBOM 和构建来源证明，并按不可变的平台 digest 运行 Docker smoke 检查。
- 首次正常启动会创建供应商扣款账本表。无需手动修改配置。

完整发布说明：[`docs/releases/v1.0.8.md`](docs/releases/v1.0.8.md)。

## [1.0.8-beta11] - 2026-08-02 - 修复自更新兼容性

- Beta 通道会按数字比较紧凑版本后缀。`beta10` 和 `beta11` 现在排在 `beta9` 之后。
- 启动时可以识别 beta8 写入的数字格式 `.update-pending` 文件。面板会把它转换为当前恢复格式，使用 SHA-256 检查新旧二进制文件，并保留原有回滚保护。
- 发布构件时会短暂等待新建的 draft release 出现在 GitHub API 中。短暂的 API 可见性延迟不会再中断正常的发布任务。
- 损坏的恢复文件仍会阻止启动，避免在更新状态不明时继续运行。
- 无需迁移数据库或手动修改配置。
- 如果服务器在从 beta8 更新到 beta9 后反复重启，请使用 beta11 发布说明中的控制台命令更新。安装程序会替换发布文件，同时保留面板数据和配置。

完整发布说明：[`docs/releases/v1.0.8-beta11.md`](docs/releases/v1.0.8-beta11.md)。
...
- Core 现在会安全回滚部分启动状态，并避免并发关闭竞争。Clash 日志流恢复工作，panic 与 fatal 保留原有行为，WireGuard endpoint 删除使用正确的 IPC 同步方式。
- 证书续期会重试已保存但应用失败的证书。Telegram 备份在派生密钥前验证固定 Argon2 参数，支持取消，并以完整的一代同时发布证书和私钥。
- 分享链接与 Clash 解析器会拒绝损坏的输入，不再崩溃或静默改写。生成的 URL 会正确转义查询参数和 IPv6 地址。
- 修复了会话、实时推送、HTTPS 重定向、iframe 安全头、域名验证与静态资源缓存的边界问题。
- WebSocket、CSRF、数据加载、备用轮询、更新轮询和编辑器状态不再受过期或重叠异步任务影响。损坏的已保存 JSON 或被禁用的浏览器存储不会再中断相关页面。
- 发布工作流会更严格地验证 tag 来源与构件元数据。更新器在更新被中断或并发执行时保留回滚状态。
- 应用正常启动时会为数据库增加持久化的供应商扣款记录表，无需手动修改配置。

完整发布说明：[`docs/releases/v1.0.8-beta9.md`](docs/releases/v1.0.8-beta9.md)。
...
- 修复数据库恢复测试的同步问题，race 测试不再产生错误的并发恢复失败。
...
- 选择性备份从同一个 source snapshot 读取，并在导出前检查 SQLite integrity、foreign keys 和 TLS sentinel。
- Traffic、IP monitor、token-use 和 audit batches 会保留 pending 数据直到数据库 commit 成功。写入失败会重试或重新入队。
...
- Go 已更新至 1.26.5，`quic-go` 更新至 0.59.1，Axios 更新至 1.18.1，受影响的 `brace-expansion` 包也已更新到包含本次安全修复的版本。
- 自更新程序现在会拒绝过大的下载、校验和响应及归档成员。下载或解压失败后，残留的部分文件会被删除。
- Telegram API 响应现在有固定大小限制，异常响应无法再让面板无限占用内存。
- IP 证书签发在停止服务后会继续执行，报告命令结果，尝试恢复服务原有状态，并返回管理菜单。
- 损坏的 Clash 代理组模板现在会返回错误，不再因访问不存在的元素而触发 panic。
- 安全检查失败会阻止构建，扫描器版本也已固定。发布任务会验证校验和、扫描产物、生成 SPDX SBOM，并附加构建来源证明。
- 无需迁移数据库。

完整发布说明：[`docs/releases/v1.0.8-beta1.md`](docs/releases/v1.0.8-beta1.md)。

## [1.0.7] - 2026-07-20 - 管理菜单与证书清理

- 当 systemd 以非零状态返回 `activating` 时，Ubuntu 管理菜单不再退出。服务已停止或不存在时，菜单也会继续运行。
- 菜单项 11 现在会在清除域名、监听地址和 Web URI 的同时，清除面板及订阅使用的 TLS 路径。证书文件和 IP 证书设置保持不变。
- Sudoku 客户端区域不再为自动管理的密钥显示重置按钮。切换客户端时，已显示的密钥会重新隐藏，Split Private Key 标签也改用界面本地化文本。
- 删除过时的手动 split key 指南。Sudoku 客户端密钥现在由面板自动生成和维护。

完整发布说明：[`docs/releases/v1.0.7.md`](docs/releases/v1.0.7.md)。
